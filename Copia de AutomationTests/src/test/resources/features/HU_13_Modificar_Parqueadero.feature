Feature: Como usuario Quiero crear un Parqueadero Para iniciar su gestión.
  ...
  Background: Un usuario ingresa credencial correctamente
#    And Ingresa a la pantalla de Administrar Parqueaderos
#    And La Unidad inmobiliaria cuenta con parqueaderos para Crear, dependiendo del número ingresado en la Creación de la Unidad Inmobiliaria.
#    And Da clic sobre el botón Crear Parqueadero


  Scenario: 1.Información que se presentaa al seleccionar el botón de crear
    Given Un usuario en la página de Administrar Unidaad Independiente
    When Seleccionaa el botón de Modificar Parqueadero
    Then El sistema presentaa un formulario “Crear Parqueadero”
    And El botón de guardaar bloqueado

  Scenario Outline: 2.Salir del formulario de Creación de Parqueadero
    Given  Un usuario en la página de Crear Parqueadero
    When   Selecciona una de las <opcion> para salir de formulario de crear Parqueadero
    Then   El sistema presentaa un <mensaje> junto con los botones Si y No
    And    La opción cerrar mensajee

    Examples:

      |opcion                            |mensaje                                                      |
      |pagina del menu                   |Esta seguro de que desea salir de la pagina crear Parqueadero|
      |enlace de la ruta de navegacion   |Esta seguro de que desea salir de la pagina crear Parqueadero|
      |icono de cerrar sesión            |Esta seguro de que desea salir de la pagina crear Parqueadero|

  Scenario Outline: 3.Cerrar el mensaje ¿Está seguro de que desea salir de la página crear Parqueadero?
    Given  Un usuario en la pagina de Modificar Parqueadero
  ...
  Scenario Outline: 4.No ingresar información en los campos requeridos
    Given  Un usuario en la página de modificar Parqueadero
...
#    Given Un usuario en la página de modificar Parqueaderoo
  ...
  Scenario Outline: 8.Se habilita botón de guardar
    Given  Un usuario en la págiina de modificar Parqueadero
  ...
  Scenario: 11.Errores al seleccionar el botón guardar
    Given  Un usuario en la página de modificar Parqueadero
  ...
  Scenario: 12.Error 400 al seleccionar el botón guardar
    ...
    And   Seleccionaa el botón de modificar
    And   Ingresaa la información en cada uno de los campos requeridos de forma correcta
    When  Daa clic en el botón Guardar
